...
import { BRAND_GAP_AFTER_ROW_BY_SIZE, panelLayout, placementRectMm } from './bagLayout'
  ...
  async function exportProductionPack(run: AdminRun) {
    ...
    try {
      ...
      for (const booking of approvedBookings) {
        ...
        const brandGapAfterRow = isFace
          ? (BRAND_GAP_AFTER_ROW_BY_SIZE[bag.name as BagRun['size']] ?? null)
          : null
        const layout = panelLayout(
          { cols, rows: rowsCount, widthMm: panelWidthMm },
          bag.height_mm,
          [],
          brandGapAfterRow,
        )
        const placement = placementRectMm(
          layout,
          booking.top_row,
          booking.left_col,
          booking.width_cells,
          booking.height_cells,
        )
        const xMm = placement.xMm
        const yMm = placement.yMm
        const artworkWidthMm = placement.widthMm
        const artworkHeightMm = placement.heightMm
        ...
        rows.push([
          ...
          panelWidthMm.toFixed(1),
          bag.height_mm.toFixed(1),
          layout.horizontalGapMm.toFixed(2),
          brandGapAfterRow === null ? '' : String(brandGapAfterRow + 1),
          xMm.toFixed(2),
          yMm.toFixed(2),
          artworkWidthMm.toFixed(2),
          artworkHeightMm.toFixed(2),
          filename,
        ])
      }
      ...
      zip.file(
        ...
        [
          ...
          '- Base selectable advertising unit: 30 × 30 mm.',
          '- These coordinates are generated by the exact same shared layout calculation used by the live 3D bag selector.',
          '- Spacing is adaptive: do not assume a fixed 33 mm pitch. The manifest records the horizontal gap used for each placement.',
          '- Front/back layouts include the same centred FreePack brand gap shown in the live preview.',
          '- A multi-cell advert is exported as one continuous rectangle spanning the selected cells and the spacing between them.',
          '- x/y and artwork dimensions in manifest.csv are the source-of-truth placement values for this FreePack layout.',
          '',
          'IMPORTANT',
          'This pack is a production placement pack, not the final manufacturer dieline.',
          'The on-screen safe boundary is a planning guide and is not yet a manufacturer-approved print-safe area.',
